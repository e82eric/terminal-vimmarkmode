...
#undef ON_ALL_ACTIONS
    ...
    bool TerminalPage::_MoveFocus(const FocusDirection& direction)
    {
        if (const auto tabImpl{ _GetFocusedTabImpl() })
        {
            if (tabImpl->FloatingPaneVisible())
            {
                if (direction == FocusDirection::Right)
                {
                    tabImpl->SelectNextFloatingPane();
                }
                else if (direction == FocusDirection::Left)
                {
                    tabImpl->SelectPreviousFloatingPane();
                }
                _ensureFloatingPaneState(tabImpl);
            ...
            if (const auto tabImpl{ _GetFocusedTabImpl() })
            {
                if (const auto pane{ tabImpl->GetActivePane() })
                {
                    ...
                    if (focusedTab->FloatingPaneVisible())
                    {
                        ...
                        focusedTab->RemoveFloatPane(pane);
                        isFloatingPane = true;
                    }
    ...
    void TerminalPage::_QuickSelect(const Tab& tab, std::wstring_view input, bool copy)
    {
        if (const auto& control{ tab.GetActiveTerminalControl() })
        {
