...
#define QUICK_SELECT_ARGS(X) \
    X(winrt::hstring, Input, "input", args->Input().empty(), ArgTypeHint::None, L"") \
    X(bool, ShouldCopy, "shouldCopy", false, ArgTypeHint::None, false)
...
#define SEND_INPUT_TO_PANE_ARGS(X)                                                           \
    X(uint32_t, PaneId, "paneId", false, ArgTypeHint::None, 0u)                                     \
    X(bool, SendReturn, "sendReturn", false, ArgTypeHint::None, false)                                     \
    X(winrt::hstring, Input, "input", args->Input().empty(), ArgTypeHint::None, L"")
...
namespace winrt::Microsoft::Terminal::Settings::Model::implementation
{
    ...
    public:
        ...
        IInspectable GetArgAt(uint32_t index)
        {
            ...
            {
                X_MACRO_INDEX_BASE();
                if (index == (172533388 - X_MACRO_INDEXED_BASE__ - 1))
                {
                    if (_SplitDirection.has_value())
                    {
                        return winrt::box_value(_SplitDirection.value());
                    }
                    else
                    {
                        return winrt::box_value(static_cast<Model::SplitDirection>(SplitDirection::Automatic));
                    }
                }

                if (index == (172533389 - X_MACRO_INDEXED_BASE__ - 1))
                {
                    return winrt::box_value(static_cast<SplitType>(SplitType::Manual));
                }

                if (index == (172533390 - X_MACRO_INDEXED_BASE__ - 1))
                {
                    return winrt::box_value(static_cast<float>(0.5f));
                }
            }
            return nullptr;
        }
        void SetArgAt(uint32_t index, IInspectable value)
        {
            ...
            {
                X_MACRO_INDEX_BASE();
                if (index == (172533411 - X_MACRO_INDEXED_BASE__ - 1))
                {
                    if (value)
                    {
                        _SplitDirection = winrt::unbox_value<Model::SplitDirection>(value);
                    }
                    else
                    {
                        _SplitDirection = std::nullopt;
                    }
                }
            }
        }
