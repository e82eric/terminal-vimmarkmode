...
void AtlasEngine::SetYankSelectionBackground(const COLORREF color) noexcept
{
    const u32 selectionColor = (color & 0xffffff) | 0xff000000;
    if (_api.s->misc->yankSelectionColor != selectionColor)
    {
        auto misc = _api.s.write()->misc.write();
        misc->yankSelectionColor = selectionColor;
        // Selection Foreground is based on the default foreground; it is also updated in UpdateDrawingBrushes
        misc->yankSelectionForeground = 0xff000000 | ColorFix::GetPerceivableColor(misc->foregroundColor, color, 0.5f * 0.5f);
    }
}
