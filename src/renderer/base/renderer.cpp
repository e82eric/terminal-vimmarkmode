    ...
    for (const auto& dirtyRect : dirtyAreas)
    {
        ...
        for (auto row = redraw.Top(); row < redraw.BottomExclusive(); row++)
        {
            ...
            _PaintBufferOutputHelper(
                pEngine,
                it,
                smIt != quickSelectState.selectionMap.end() ? smIt->second : decltype(smIt->second)(), screenPosition);
...
[[nodiscard]] HRESULT Renderer::_PrepareRenderInfo(_In_ IRenderEngine* const pEngine)
{
    ...
    info.vimCursorBackgroundColor = _renderSettings.GetColorTableEntry(TextColor::BRIGHT_YELLOW);
